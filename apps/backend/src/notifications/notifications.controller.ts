...
export class NotificationsController {
  ...
  async getAll(@Request() req: any) {
    return this.notifService.getNotifications(req.user.userId);
  }
  ...
  async markAsRead(@Param('id') id: string, @Request() req: any) {
    const notif = await this.notifService.markAsRead(id, req.user.userId);
  ...
  async remove(@Param('id') id: string, @Request() req: any) {
    const deleted = await this.notifService.remove(id, req.user.userId);
    return { deleted };
  }
  ...
  async removeAll(@Request() req: any) {
    await this.notifService.removeAllForUser(req.user.userId);
